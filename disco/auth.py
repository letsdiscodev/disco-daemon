...
def get_api_key_sync(
    ...
    bearer_credentials: Annotated[HTTPAuthorizationCredentials| None, Depends(bearer_header)],
    ...
    if basic_credentials is not None:
        ...
        if unverified_jwt is not None:
            ...
            api_key_for_public_key = get_api_key_by_public_key(dbsession, public_key)
            if api_key_for_public_key is not None:
                try:
                    jwt.decode(bearer_jwt, api_key.id, algorithms="HS256")
                    api_key_str = api_key_for_public_key.id
                except jwt.PyJWTError:
                    pass
    if api_key_str is None:
        raise HTTPException(status_code=401)
    api_key = get_valid_api_key_by_id_sync(dbsession, basic_credentials.username)
    if api_key is None:
...
async def get_api_key(
    basic_credentials: Annotated[HTTPBasicCredentials, Depends(basic_header)],
    bearer_credentials: Annotated[HTTPAuthorizationCredentials, Depends(bearer_header)],
    dbsession: Annotated[AsyncDBSession, Depends(get_db)],
):
    api_key = await get_valid_api_key_by_id(dbsession, basic_credentials.username)
    if api_key is None:
        raise HTTPException(status_code=403)
