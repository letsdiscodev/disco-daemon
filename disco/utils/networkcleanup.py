...
And then, a few days later, delete the network. The delay is to let the
"docker service rm" command to take full effect (the call just tells Docker Swarm
to start getting rid of the service, but the actual effect is async) and to let
the raft data propagate.
...
async def remove_unused_networks() -> None:
    # Behind feature flag. To enable, run against the SQLite DB:
